...
const Register:React.FC<RegisterComponentProps> = () => {
  ...
  const handleSubmit = async(e:React.SyntheticEvent) => {
    e.preventDefault();
    alert()
    const { username, firstname, lastname, phoneno, password, profilepicture, age, email } = formData;
    try {
        const { data } = await createUser({
          variables: { username, firstname, lastname, phoneno, password, profilepicture, age, email },
        });
      ...
      <div className="bg-white p-8 rounded-md shadow-md w-full max-w-md">
        ...
        {formData.profilepicture && (
              <img
                ...
                alt="Profile Preview"
                className="mt-2 rounded-md shadow-md" style={{ width: '80px', height: '80px', objectFit: 'cover', border:'10px' ,borderRadius:'30%'}}
              />
            )}
            </div>
        <form >
